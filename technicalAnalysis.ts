import { type CandleData, type TechnicalIndicators, type TradingSignal, type SignalStrength, type SignalType, type Timeframe } from "@shared/schema";
...
export class TechnicalAnalysis {
  ...
  static heikinAshi(data: CandleData[]): CandleData[] {
  ...
  static calculateIndicators(data: CandleData[]): TechnicalIndicators | null {
  ...
  // Generate trading signals based on LUPOWN strategy
  static generateSignals(data: CandleData[], symbol: string, timeframe: Timeframe): Omit<TradingSignal, 'id' | 'timestamp'>[] {
    if (data.length < 200) return [];

    const closes = data.map(d => d.close);
    const signals: Omit<TradingSignal, 'id' | 'timestamp'>[] = [];
    ...
    if (recentIndex >= 200) {
      ...
      // EMA crossover signals (LUPOWN strategy core)
      if (prevEMA10 <= prevEMA55 && currentEMA10 > currentEMA55) {
        const strength: SignalStrength = currentEMA100 > currentEMA200 ? 'STRONG' : 'MODERATE';
        signals.push({
          symbol,
      ...
      if (currentRSI < 30 && currentEMA10 > currentEMA55) {
        signals.push({
          symbol,
          type: 'BUY' as SignalType,
      ...
      if (currentRSI > 70 && currentEMA10 < currentEMA55) {
        signals.push({
          symbol,
          type: 'SELL' as SignalType,
          strength: 'WEAK' as SignalStrength,
